...
Global BHOPT := False, RapidFireT := False, TriggerBotT := False, TurnAroundT := False, AcceptT := False, CrosshairT := False, DuckT := False, RCSNotification := True, TriggerBotNotification := True

Version := "CS2 V1.2"
/*
Added some lines for performance
Adjusted M4A4 Spray (WIP)
Adjusted Saving For Magnifier and Crosshiar. They no longer check for hotkeys or min and max for other features.
Adjusted Extras GUI Tab
Added PixelBot Notification 
Added Independent save for Extras
Updated how CrossHair is Created&Destroyed, Also how its called to.
Updated Zoom to delete again for smoother zoom.
Added Traymenu Icon / Failsafe
Added Traymenu Option
Added Picture check function
Updated Pixelbot Notification not closing or opening under certian conditions.
*/
...
if !FileExist("Settings.ini") {
	...
	Menu, Tray, Icon, FBI_Evee.PNG, ,1
		...
		MsgBox, 49, ERROR!!!, FBI_Evee.PNG IS MISSING!`nSTART THE HELICOPTER WE NEED TO FIND AGENT EVEE!!
		ifMsgBox ok
		{
		PictureCheck("FBI_Evee.png")
		}
		IfMsgBox Cancel
		{
...
Gui, Add, CheckBox, xS+5 ys+70 Checked%BHOPT% gBHOP_CB, BHOP
Gui, Add, Hotkey, xS+100 ys+65 w80 vKey_BHOP, %Key_BHOP%
Gui, Add, CheckBox, xS+5 ys+110 Checked%DuckT% gDuck_CB, Crouch Correction
Gui, Add, CheckBox, xS+5 ys+150 Checked%RCSNotification% gRCSNotification_CB, RCS Notification
Gui, Add, CheckBox, xS+5 ys+180 Checked%TriggerBotNotification% gTriggerBotNotification_CB, PixelBot Notification
Gui Add, Button, xS+20 ys+270 w250 h30 gSelectProcess, Real Time Priority
Gui Add, Button, xS+20 ys+305 w250 h30 gSaveExtras, Save Extras
Gui, Submit, NoHide
...
if (GuiVisible) {
 ...
 Gosub DuplicateCheck
	global reaction_min := ReactionMin, reaction_max := ReactionMax 
	MsgBox, 4, Do you want to Save?, TurnAround Toggle: %TurnAroundT%`nTurn Around Key: %key_180%`nBHOP Toggle: %BHOPT%`nBHOP Key: %Key_BHOP%`nCrouch Correction Toggle: %DuckT%`nRCS Notification: %RCSNotification%`nPixelBot Notification: %TriggerBotNotification%
IfMsgBox Yes
		CustomSave(Key_180,"Hotkeys", "Key_180")
...
MainLoop() { ; No seriously stop lurking. Everything below is naked. You wouldnt peep on a naked women would you? Thats what you'll be doing if you go any lower!
...
DllCall("mouse_event", "UInt", 0x01, "UInt", -14/humanizer88*modifier, "UInt", 11/humanizer88*modifier)
sleep 88/waitdivider88
DllCall("mouse_event", "UInt", 0x01, "UInt", -14/humanizer88*modifier, "UInt", 11/humanizer88*modifier)
sleep 88/waitdivider88
DllCall("mouse_event", "UInt", 0x01, "UInt", -14/humanizer88*modifier, "UInt", 11/humanizer88*modifier)
...
DllCall("mouse_event", "UInt", 0x01, "UInt", 10/humanizer*modifier, "UInt", 4/humanizer*modifier)
sleep 87/waitdivider
DllCall("mouse_event", "UInt", 0x01, "UInt", 10/humanizer*modifier, "UInt", 4/humanizer*modifier)
sleep 87/waitdivider
DllCall("mouse_event", "UInt", 0x01, "UInt", 10/humanizer*modifier, "UInt", 4/humanizer*modifier)
...
DllCall("mouse_event", "UInt", 0x01, "UInt", 12/humanizer88*modifier, "UInt", 2/humanizer88*modifier)
sleep 88/waitdivider88
DllCall("mouse_event", "UInt", 0x01, "UInt", 12/humanizer88*modifier, "UInt", 2/humanizer88*modifier)
sleep 88/waitdivider88
DllCall("mouse_event", "UInt", 0x01, "UInt", 12/humanizer88*modifier, "UInt", 2/humanizer88*modifier)
...
DllCall("mouse_event", "UInt", 0x01, "UInt", -15/humanizer*modifier, "UInt", 0/humanizer*modifier)
sleep 87/waitdivider
DllCall("mouse_event", "UInt", 0x01, "UInt", -15/humanizer*modifier, "UInt", 0/humanizer*modifier)
sleep 87/waitdivider
DllCall("mouse_event", "UInt", 0x01, "UInt", -15/humanizer*modifier, "UInt", 0/humanizer*modifier)
...
;Welp you did it, you lurked till the end. Hope I was able to show you something new.